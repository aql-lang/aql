...
const refusalCeiling = 425 // P0 651 -> P3 642 -> wide poly 618 -> P4 CALL_DYNAMIC (fn-value apply) 616 -> P5 multi/0-result calls 598 -> make carrier-identity (impure constructor fresh-id) 580 -> value-def locals (OpStoreLocal) 568 -> dup carrier-identity (distinct ids for repeated outputs) 565 -> predicate-type operand provenance (DepScalar const-bake) 555 -> if value-else arm 545 -> case clause compilation (desugar to if-chain) 542 -> multi-return / 0-return / anonymous-lambda fns (N-result fn units) 538 -> apply of a fn value (ReturnsIdentity makes the check engine re-step it) 527 -> unnamed-fn map/list members const-bakeable (method fields: m.f via poly get + CALL_DYNAMIC) 521 -> 0-value-then 2-arg if statement guard (if cond [raise]) 519 -> concrete-args dynamic-output core builtins bake CALL_NATIVE (unify et al) 514 -> module inner natives (StructUtil.* etc, dot-access) bake CALL_NATIVE via sub-registry sig verification 459 -> 3-arg operand shape: sig-0 result over const operands lowers via push+swap chain (setpath et al) 453 -> strict-disjunct straddle lowers OpCallNativePoly (5 is (tnot (Integer gt 0))) 445 -> object/class atom-keyed set (quoted-operand exemption, mutation-safe) 425
