...
const islandCeiling = 10 // atom-keyed gets 102->36; fold-no-init + filter (dynamic-output) closures 36->29; P5 multi/0-result calls 29->26; case clause compilation (islanded cases now compile natively) 26->15; map-iteration quotation each/fold/scan over {…} compile to closures via InvokeBody (roadmap item 5b) 15->9; +1 (9->10): baking inert reach lens values let `filter $.on {map}` cross from REFUSED to compiled-with-island (a net gain — a previously-uncompilable row now compiles, NOT a native->island regression); lower as more island shapes compile natively
