...
const islandCeiling = 3 // atom-keyed gets 102->36; fold-no-init + filter (dynamic-output) closures 36->29; P5 multi/0-result calls 29->26; case clause compilation (islanded cases now compile natively) 26->15; map iteration (each/fold/filter over a map) compiles a value-body closure via InvokeBody 15->9; nested-variadic branch lowering compiles no-default case islands 9->7; empty-body each/scan compiles natively (a 0-net iteration body is the handler's OWN each_error/scan_error, raised byte-identically from InvokeBody, so EmptyBodyErrors compiles the body as a count-agnostic closure instead of refusing it and islanding) 7->3; lower as more island shapes compile natively
