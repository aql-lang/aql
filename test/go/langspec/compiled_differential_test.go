...
// documented decision.
const minCompiledRows = 1894 // + F4 class/object make (plain-data const-bake) + get islands + fn-value boundary + is/typeof on make-result (type-operand ID-collision guard): 1636 compiled June 2026; + P5 multi/0-result calls: 1677 compiled; + multi-return / 0-return / anonymous-lambda fns (N-result fn units): 1742; + apply of a fn value (check-engine re-step): 1753; + unnamed-fn map/list members (method fields m.f): 1759; + 0-value-then if statement guard: 1760; + concrete-args dynamic-output core builtins (unify et al): 1765; + module inner natives (dot-access): 1813; + 3-arg operand shape (sig-0 result over consts, push+swap chain): 1820; + strict-disjunct type-algebra poly (is/tand over tnot-predicate): 1825 (the differential count is sensitive to test grouping — this is the strict CI-group value; the full make-test grouping runs a few higher); + object/class atom-keyed set: 1839; + computed-else if (OpDrop): 1843; + variadic-else statement-if: 1847; + fn-value introspection (baked fn-value const): 1857; + map-iteration quotation closures + filter list-lambda named-param closure (roadmap items 5b/5a-1): 1864; + map lambdas via KeyVal-input closures (item 5a-2): 1870; + with-decimal body closure (item 7): 1872; + aql:query DSL const-baked clause/table operands (item 7): 1891; + reach inert paths + raise atom: 1894 (strict isolated-run value; full make-test grouping runs a few higher)
