...
var frontierCompileLedger = map[string]frontierEntryLS{
	...
	`fold ([e:Integer a:Integer] => [a mul 10 add e]) [1 2 3] 0`:                                                                                                                                             {why: "the NON-commutative list fold: a compiled closure binds inputs positionally while MatchFnSig consumes the stack top-down, so a 2-input callback arrives swapped (123 interpreted, 60 positional); graduation = a callback frame that reproduces the ASSIGNMENT, not more carriers", failsWith: "islanded: program embeds an OpFallback span"},
	`scan ([e:Integer a:Integer] => [a mul 10 add e]) [1 2 3]`:                                                                                                                                               {why: "the scan twin of the ordering row — same 2-input assignment gap", failsWith: "islanded: program embeds an OpFallback span"},
