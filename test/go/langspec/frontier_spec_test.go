...
var frontierCompileLedger = map[string]frontierEntryLS{
	...
	`def fs [ (a:Integer => [add 1 a]) (a:Integer => [mul 2 a]) ] end ((fs get 1) 5)`:                               {why: "NUR101 (ruled 2026-08-26): the paren PLACES the retrieved fn \u2014 `get` is not one of ADR-011's three application forms (a bare name, `apply`, a member read) \u2014 so the residual is [fn, arg] and the auto-dispatch guard declines it. Parity holds through the fallback; graduation is the guard learning that a PLACED lead cannot auto-apply, which needs the placement signal to be positional rather than value-ID-keyed", failsWith: "fn value precedes residual args"},
	`def tbl {double:(a:Integer => [mul 2 a]) inc:(a:Integer => [add 1 a])} end def k 'double' end ((tbl get k) 5)`: {why: "NUR101 (ruled 2026-08-26): same family as the list row, but it declines at a DIFFERENT site \u2014 recordParenLeadingApply, the paren-bounded method window, because `tbl get k` carries member-read provenance. Before the gate there it MISCOMPILED to 10 against a placed `fn (Integer) 5`. Refusing is the parity-preserving answer", failsWith: "fn-value application bounded by a paren"},
	`def mk fn a:Integer Function [(fn b:Integer Integer [add a b])] end ((mk 1) 2)`:                                {why: "NUR101 (ruled 2026-08-26): a statement no longer applies a collapsed fn value; the paren places it uniformly, so the residual is [fn, 2] and the auto-dispatch guard declines. Was pinned as compiling to 3", failsWith: "fn value precedes residual args"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end def cor p:Function => [q:Function => [q/v (ctrue/v p/v apply) apply]] end 'F' ('T' (cif (cfalse/v (cor ctrue/v) apply)) apply) apply`:  {why: "audit §5.8: Church or, T or F. RE-DIAGNOSED 2026-08-26 under NUR101's place-uniformly ruling: the staged spelling no longer draws check diagnostics — `boru check` is clean (0 errors, dynamic(Any)) — because placement changed what cor's body nets, so the refusal MOVED from the check pass to the recorder. Still refused, same interpreted answer; only the mode changed", failsWith: "body result of unknown provenance"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end def cor p:Function => [q:Function => [q/v (ctrue/v p/v apply) apply]] end 'F' ('T' (cif (cfalse/v (cor cfalse/v) apply)) apply) apply`: {why: "audit §5.8: Church or, F or F. RE-DIAGNOSED 2026-08-26 under NUR101's place-uniformly ruling: the staged spelling no longer draws check diagnostics — `boru check` is clean (0 errors, dynamic(Any)) — because placement changed what cor's body nets, so the refusal MOVED from the check pass to the recorder. Still refused, same interpreted answer; only the mode changed", failsWith: "body result of unknown provenance"},
