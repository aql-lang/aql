...
// lang/doc/design/TYPE-UNIFORM.0.md. `type BaseType arg` constructs
...
func TestTypeRecord(t *testing.T) {
	got := runOne(t, "def Pt (type Record [x:Integer y:Integer])\nmake Pt {x:3 y:4} .x")
	if len(got) != 1 || got[0] != int64(3) {
		t.Errorf("got %v, want [3]", got)
	}
}

// A record takes a list, not a map — `type Record {…}` is rejected.
func TestTypeRecordRejectsMap(t *testing.T) {
	a, err := lang.New()
	if err != nil {
		t.Fatalf("new: %v", err)
	}
	if _, err := a.Run("type Record {x:Integer}"); err == nil {
		t.Fatal("expected error for `type Record {x:Integer}` (map), got nil")
	}
}

// A type-built record renders as a record type.
func TestTypeRecordRenders(t *testing.T) {
	got := runOne(t, "def Pt (type Record [x:Integer y:String])\nPt")
...
func TestTypeTable(t *testing.T) {
	got := runOne(t, "def Row (type Record [n:String])\n"+
		"def Tbl (type Table Row)\n"+
...
func TestTypeObjectInheritance(t *testing.T) {
	got := runOne(t, "def Animal (type Object {legs:Integer})\n"+
		"def Dog (type Animal {breed:String})\n"+
...
func TestTypeBadBase(t *testing.T) {
	...
	if _, err := a.Run("type Integer {x:1}"); err == nil {
		t.Fatal("expected error for `type Integer {x:1}`, got nil")
	}
}
