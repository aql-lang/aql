...
// {a:"hello"} StructUtil.transform {x:"`a`"} — injects value from data into spec
func TestTransformInject(t *testing.T) {
	bt := string(rune(96)) // backtick character
	input := `{a:"hello"} StructUtil.transform {x:"` + bt + `a` + bt + `"}`
...
// {a:"1"} StructUtil.transform {x:99} — literal spec passthrough
func TestTransformPassthrough(t *testing.T) {
	result, err := runNativeSteps(t, nil, []string{
		`{a:"1"} StructUtil.transform {x:99}`,
	})
	if err != nil {
		t.Fatal(err)
	}
	m, _ := native.AsMap(result[0])
	v, _ := m.Get("x")
	vi1, _ := native.AsInteger(v)
	if vi1 != 99 {
		t.Errorf("expected 99, got %d", vi1)
	}
}

// StructUtil.transform with nested path: {a:{b:42}} StructUtil.transform {val:"`a.b`"}
func TestTransformNestedPath(t *testing.T) {
	bt := string(rune(96)) // backtick character
	input := `{a:{b:42}} StructUtil.transform {val:"` + bt + `a.b` + bt + `"}`
	result, err := runNativeSteps(t, nil, []string{input})
	if err != nil {
		t.Fatal(err)
	}
	m, _ := native.AsMap(result[0])
	v, _ := m.Get("val")
	vi2, _ := native.AsInteger(v)
	if vi2 != 42 {
		t.Errorf("expected 42, got %d", vi2)
	}
}

// foo load {id:"1"} StructUtil.transform {greeting:"`name`"} — chained prefix
