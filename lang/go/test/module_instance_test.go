...
func TestModuleInstanceFileModule(t *testing.T) {
	files := map[string]string{
		"proj/lib.aql": `export "Lib" {answer: 42}`,
	}
	run := func(step string) eng.Value {
		t.Helper()
		res, err := runModuleSteps(t, files, []string{`import "./proj/lib.aql"`, step})
		if err != nil {
	...
	// file ends in proj/lib.aql; folder ends in proj.
	if s, _ := native.AsString(run(`Lib.$module.file`)); !strings.HasSuffix(s, "proj/lib.aql") {
		t.Errorf("Lib.$module.file = %q, want suffix proj/lib.aql", s)
	}
