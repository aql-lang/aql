...
func TestModuleFnCheckPathGate(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			`"aql:test" import end  def double fn [[n:Integer][Integer][n 2 mul]] end ` +
		...
		},
		{
			...
			`"aql:test" import end  def inc fn [[n:Integer][Integer][n 1 add]] end ` +
		...
		},
		{
			...
			`"aql:test" import end  def run-case fn [[x:Integer][Integer][x 100 add]] end ` +
