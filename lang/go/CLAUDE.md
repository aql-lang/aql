...
> explicit `apply` — it is settled: `apply` now applies a fn whose only
> signatures are 0-arg at the apply site itself, so
> `def f ([] => [42])  f/v apply` answers `42` exactly as its named twin
> does. See `design/FN-VALUE-OPEN-WORK.0.md` §5 Hole 1; pinned in
> `lang/spec/valof.tsv` §5, whose heading ("Anonymous-function references
> behave identically") was the one claim in that file which did not hold
> at arity 0.
>
