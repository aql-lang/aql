...
// `parse` kind by boru:parselang (see modules/parselang.go), so
// `import "boru:parselang"` then `parse aontu '<text>'` parses, unifies, and
...
// is the decoder the boru:parselang `aontu` kind is built from.
//
