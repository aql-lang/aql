...
func mmapVal(t *testing.T, r *Registry, path string, opts func(*OrderedMap)) Value {
	t.Helper()
	res := runAQL(t, r, []Value{NewWord("mmap"), pathV(path), wrapMap(opts)})
...
func TestMmapWord(t *testing.T) {
	...
	txt := runAQL(t, r, []Value{NewWord("read"), m})
	if s, _ := AsString(txt[0]); s != "hello" {
		t.Errorf("mmap read text = %q", s)
	}
	b := runAQL(t, r, []Value{NewWord("read"), m, wrapMap(func(om *OrderedMap) { om.Set("enc", NewString("bytes")) })})
	if bs, ok := AsBytesValue(b[0]); !ok || string(bs) != "hello" {
		t.Errorf("mmap read bytes = %v", b[0])
	}
	win := runAQL(t, r, []Value{NewWord("read"), m,
	...
	runAQL(t, r, []Value{NewWord("close"), m})
	...
	runAQL(t, r, []Value{NewWord("write"), w, NewBytesValue([]byte("HE"))})
	runAQL(t, r, []Value{NewWord("flush"), w})
	...
	wb := runAQL(t, r, []Value{NewWord("read"), ww})
	if s, _ := AsString(wb[0]); s != "Ell" {
		t.Errorf("windowed mmap region = %q", s)
	}
	runAQL(t, r, []Value{NewWord("close"), ww})

	// mmap of an absent path errors.
	if err := runAQLError(t, r, []Value{NewWord("mmap"), pathV("ghost")}); err == nil {
...
func TestMmapWriteRefusals(t *testing.T) {
	...
	if err := runAQLError(t, r, []Value{NewWord("write"), ro, NewBytesValue([]byte("x"))}); err == nil {
		t.Error("write to a read-only region should refuse")
	}
	// A write past the region cannot grow the mapping.
	w := mmapVal(t, r, "m.bin", func(om *OrderedMap) { om.Set("writable", NewBoolean(true)) })
	if err := runAQLError(t, r, []Value{NewWord("write"), w, NewBytesValue([]byte("toolong!!")),
	...
	res := runAQL(t, r, []Value{NewWord("write"), w, NewBytesValue([]byte("Z")),
