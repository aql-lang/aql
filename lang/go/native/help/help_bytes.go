...
// Binary FRAME layouts are a separate concern: `Packet` is the spec KIND
// (sibling of Object/Record), and a frame type refines it —
// `def Header (refine Packet [layout])`, then `make Header {fields}` packs to a
// plain Bytes and `unpack`/`unpack-prefix` decode (ADR-007 — the layout is
// plain Node data on the type, not a parsed spec). See
// design/go-modules/BYTES.10.md. Heavier
...
func init() {
	register(&Entry{
		...
		Description: "`unpack-prefix <FrameType> b` matches as much of the Bytes `b` as " +
			...
			"`def P (refine Packet [layout])`, where the layout is a `List` of segment " +
		...
		Examples: []string{
			`def Msg (refine Packet [{name:'op' type:'u8'} {name:'len' type:'u16'} {name:'body' type:'bytes' size:'len'}])  unpack-prefix Msg (convert Bytes [1 0 2 104 105])   # {ok:{op:1 len:2 body:Bytes<68 69>} rest:Bytes<>}`,
			`def Hdr (refine Packet [{name:'op' type:'u8'} {name:'len' type:'u16'}])  unpack-prefix Hdr (convert Bytes [0])   # {need: 2}`,
		},
	})
}
