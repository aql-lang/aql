...
func init() {
	...
	register(&Entry{
		...
		Description: "Adds two numeric values. When both are integers the result is an integer; " +
			...
			"logical words). A cross-type pair (e.g. a Boolean and a Number) matches no " +
			"overload at all, so it raises [boru/signature_error] — operations apply " +
			"within a type, not across.",
		Notes: []string{
	...
	// --- boru:math-util native module (requires: import \"boru:math-util\") ---

	register(&Entry{
		Word:        "abs",
		Summary:     "Return the absolute value of a number.",
		Description: "Returns the absolute (non-negative) value.",
		Notes:       []string{"Requires: import \"boru:math-util\""},
	})
	...
	register(&Entry{
		...
		Notes: []string{
			...
			"Requires: import \"boru:math-util\"",
		},
	})
