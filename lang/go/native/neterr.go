...
//
// It exists so the socket layer (boru:net's connect-raw/recv/send words)
...
func MapTransportErrAs(r *Registry, word, what string, err error) error {
	...
	case errors.Is(err, io.EOF), errors.Is(err, net.ErrClosed), errors.Is(err, io.ErrClosedPipe):
		return r.BoruError("closed", what+": connection closed by peer", word)
	case errors.As(err, &nerr) && nerr.Timeout():
		return r.BoruError("timeout", what+": deadline exceeded", word)
	default:
		...
		if ContainsAny(msg, "broken pipe", "connection reset", "use of closed network connection") {
			return r.BoruError("closed", what+": "+msg, word)
		}
		return r.BoruError("transport", what+": "+msg, word)
	}
}
