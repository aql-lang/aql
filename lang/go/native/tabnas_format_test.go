...
func TestEngineReadUnknownFmt(t *testing.T) {
	...
	err = runAQLError(t, r, []Value{NewWord("read"), pathV("x.dat"), NewMap(opts)})
...
func TestEngineWriteReadOnlyFormat(t *testing.T) {
	...
	err = runAQLError(t, r, []Value{NewWord("write"), pathV("out.xml"), NewString("<r/>"), NewMap(opts)})
...
func TestWriteReadOnlyDoesNotInvokeEncode(t *testing.T) {
	...
	err = runAQLError(t, r, []Value{NewWord("write"), pathV("out.spy"), NewString("x"), NewMap(opts)})
	if err == nil || !strings.Contains(err.Error(), "read-only") {
