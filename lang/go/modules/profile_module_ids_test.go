...
// ids — so `aql policy validate` passes a typo silently, and a typo is
// double-edged: it permits a module that does not exist AND leaves the real
// one denied.
//
// That is not hypothetical. sandbox.jsonic shipped `"aql:math"` and
// `"aql:time"`, neither of which is a module id (the real ones carry the
...
// carried the same `aql:math` typo.
//
...
var externalModules = map[string]string{
	// Moved out of this repository (commit a7882da, "remove the aql:decision
	...
	"aql:decision": "moved to a separate repository; gen.jsonic names it for hosts that register it",
}
