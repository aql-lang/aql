...
func TestBinResolve(t *testing.T) {
	...
	desc, err := Resolve("bin-util", r)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	binExport, ok := desc.Exports["BinUtil"]
	if !ok {
...
func TestBinRotates(t *testing.T) {
	...
	// 1 BinUtil.rotr 1 → high bit set: -9223372036854775808
	e2 := native.New(r)
...
func TestBinSingleBit(t *testing.T) {
	r := binRegistry(t)
	// 0xa5 (= 10100101) BinUtil.test 0 → true
	e := native.New(r)
...
func TestBinMaskReverseSwap(t *testing.T) {
	...
	// 1 BinUtil.reverse → high bit set: 1 << 63
	e4 := native.New(r)
	...
	// 0x0102030405060708 BinUtil.swap → 0x0807060504030201
	e5 := native.New(r)
...
func TestBinExtractInsert(t *testing.T) {
	r := binRegistry(t)
	// 0xabcd BinUtil.extract 4 12 → 0xbc (bits [4, 12))
	e := native.New(r)
	...
	// 0xabcd BinUtil.insert 4 12 0x00 → low 4 bits unchanged (0xd), bits [4,12) zeroed
