...
// The `sx` mini-language: query AQL native structure (Maps and Lists) with
// XPath (github.com/antchfx/xpath). Where `xp` runs XPath over a Node/Xml
// document, `sx` runs the SAME XPath engine over ordinary AQL data, so the
// full axis / predicate / function vocabulary (//, [pred], count(), sum(),
// string(), contains(), position(), …) reaches into maps and lists — the
// XPath-shaped query layer the XSLT-style rule engine wants (see
// design/fmt-module-and-xslt.0.md). The document is the stack subject and
// the result is a List, the same shape as `xp` / `jp` / `jq`:
//
//	{a:1 b:2} mini sx '/a'                       # → [1]
//	{books:[{title:'A'} {title:'B'}]} mini sx '//title'   # → ['A' 'B']
//	{items:[{p:10} {p:30}]} mini sx '//item[p>20]/p'      # → [30]
//	{a:1 b:2 c:3} mini sx 'count(/*)'            # → [3]
//
...
// buildSxTree mirrors an AQL Map/List/scalar subject into the navigable
...
func buildSxTree(v native.Value) *xpNode {
	root := &xpNode{typ: xpath.RootNode}
	container := buildSxElement("", v)
...
// buildSxElement mirrors one AQL value into an element node named name. A
...
func buildSxElement(name string, v native.Value) *xpNode {
	n := &xpNode{typ: xpath.ElementNode, name: name, aql: v}
	if m, err := native.AsMap(v); err == nil {
		for _, k := range m.Keys() {
			cv, _ := m.Get(k)
			sxAppend(n, buildSxElement(k, cv))
		}
		return n
	}
	if lst, err := native.AsList(v); err == nil {
		for i := 0; i < lst.Len(); i++ {
			sxAppend(n, buildSxElement("item", lst.Get(i)))
		}
		return n
	}
	sxAppend(n, &xpNode{typ: xpath.TextNode, text: xpText(v)})
	return n
}

// sxAppend links child as the last child of parent, wiring the parent and
// prev/next sibling pointers the navigator relies on.
func sxAppend(parent, child *xpNode) {
	child.parent = parent
...
// miniSxHandler implements `sx`: compile the XPath src (args[0]), evaluate it
// over the stack Map/List subject (args[2]), and return the result as a List.
func miniSxHandler(args []native.Value, _ map[string]native.Value, _ []native.Value, r *native.Registry) ([]native.Value, error) {
	src, err := args[0].AsConcreteString()
	if err != nil {
		return nil, r.AqlError("mini_error", "sx: src: "+err.Error(), "lang_sx")
	}
	doc := args[2]
	...
	if mapErr != nil && listErr != nil {
		return nil, r.AqlErrorHint("mini_error",
			"sx: expected a Map or List, got "+doc.Parent.String(), "lang_sx",
			"the sx document is an AQL structure — a Map or a List")
	}
	expr, perr := miniCompiledXPath(src)
	if perr != nil {
		return nil, r.AqlErrorHint("mini_parse_error", "sx: "+perr.Error(), "lang_sx",
			"write an XPath like /name, //title or count(//item)")
	}
	result := expr.Evaluate(newXpNav(buildSxTree(doc)))
	return []native.Value{native.NewList(sxResultToList(result))}, nil
}

// sxResultToList projects an antchfx XPath result into AQL values: a node-set
...
// list.
func sxResultToList(result any) []native.Value {
	switch v := result.(type) {
	case *xpath.NodeIterator:
		var out []native.Value
		for v.MoveNext() {
			out = append(out, sxNodeToValue(v.Current()))
		}
		return out
	case bool:
...
// sxNodeToValue converts one matched node back to an AQL value: an element to
// its source value (map/list/scalar), a text node to its String value.
func sxNodeToValue(nav xpath.NodeNavigator) native.Value {
