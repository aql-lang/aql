...
func TestTimeWave4TzErrors(t *testing.T) {
	...
	result := runAQL(t, r, callTimeDot("tz", native.NewString("America/New_York")))
...
func TestTimeWave4UnixNs(t *testing.T) {
	r := timeRegistry(t)
	ins := runAQL(t, r, callTimeDot("unix-ns", native.NewInteger(1_500_000_000_000_000_000)))
	back := runAQL(t, r, callTimeDot("to-unix", ins[0]))
...
func TestTimeWave4UntilBorrow(t *testing.T) {
	...
	result = runAQL(t, r, callTimeDot("until", d3, d4))
	cd, ok = native.AsCalendarDuration(result[0])
...
func TestTimeWave4DurSignClock(t *testing.T) {
	...
	for _, c := range []struct {
		...
		d := runAQL(t, r, callTimeDot("hours", native.NewInteger(c.n)))
		s := runAQL(t, r, callTimeDot("dur-sign", d[0]))
	...
	for _, c := range []struct {
		...
		d := runAQL(t, r, callTimeDot("years", native.NewInteger(c.n)))
		s := runAQL(t, r, callTimeDot("dur-sign", d[0]))
		if n, err := s[0].AsConcreteInteger(); err != nil || n != c.want {
...
func TestTimeWave4Elapsed(t *testing.T) {
	r := timeRegistry(t)
	d := runAQL(t, r, callTimeDot("elapsed", native.NewInstant(time.Unix(0, 0))))
	total := runAQL(t, r, callTimeDot("total-hours", d[0]))
...
func TestTimeWave4CompareEarliestLatest(t *testing.T) {
	...
	for _, c := range []struct {
		...
		result := runAQL(t, r, callTimeDot("compare", c.d1, c.d2))
	...
	for _, c := range []struct {
		...
		result := runAQL(t, r, callTimeDot(c.word, c.d1, c.d2))
...
func TestTimeWave4StartEndOfUnits(t *testing.T) {
	...
	for _, c := range []struct {
		...
		result := runAQL(t, r, callTimeDot("start-of", c.d, native.NewString("week")))
	...
	result := runAQL(t, r, callTimeDot("start-of", wednesday, native.NewString("day")))
	...
	for _, c := range []struct {
		...
		result = runAQL(t, r, callTimeDot("end-of", c.d, native.NewString("week")))
	...
	result = runAQL(t, r, callTimeDot("end-of", wednesday, native.NewString("day")))
	if got := native.AsDate(result[0]); got.Day() != 13 {
		t.Errorf("end-of day = %v, want the 13th", got)
	}
	result = runAQL(t, r, callTimeDot("end-of", wednesday, native.NewString("quarter")))
...
func TestTimeWave4TzLocalNameOffset(t *testing.T) {
	...
	result := runAQL(t, r, callTimeDot("tz-local"))
	...
	result = runAQL(t, r, callTimeDot("tz-name", tt.NewTimezone(ny)))
	...
	result = runAQL(t, r, callTimeDot("tz-offset", winter, tt.NewTimezone(ny)))
	...
	result = runAQL(t, r, callTimeDot("tz-offset", winter, tt.NewTimezone(stj)))
...
func TestTimeWave4IsDst(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			result := runAQL(t, r, callTimeDot("is-dst", c.ins, c.tz))
