...
func init() {
	registerDocs("aql:model", map[string]string{
		"new": "Construct a system model from a spec Map: `Model.new {src:'…'}` (inline " +
			...
			"(Map), `dryrun` (Boolean), `order` (List of action names), `config` (Boolean, default " +
			"false), `watch` ({mod,add,rem}), and `actions` (a Map of name → Function, or name → " +
			"{run:Function, step:'pre'|'post'|'all'}). Returns a Model value. Wraps " +
			"github.com/voxgig/model (Go); the model is resolved via aontu unification.",
