...
func TestSetFileOps(t *testing.T) {
	...
	// read/write moved to boru:io; import once (state persists across Run).
	if _, err = a.RunInterp(`import "boru:io"`); err != nil {
		t.Fatal(err)
	}
...
func TestRunDefaultBranch(t *testing.T) {
	...
	// read moved to boru:io; import once (state persists across Run).
	if _, err = a.RunInterp(`import "boru:io"`); err != nil {
		t.Fatal(err)
	}
...
func TestRegisterAddsAlongsideBuiltin(t *testing.T) {
	...
	// `upper` moved to boru:string-util; register the string words so this test
