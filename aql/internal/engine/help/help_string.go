...
func init() {
	register(&Entry{
		...
		Description: "Converts every character in the input string to its uppercase equivalent " +
			"using Go's strings.ToUpper (Unicode-aware).",
		Examples: []string{
			`"hello" upper        => "HELLO"`,
			`"café" upper         => "CAFÉ"`,
		},
		Notes: []string{
			"Works on both string and atom values.",
		},
	})
	...
	register(&Entry{
		...
		Description: "Converts every character in the input string to its lowercase equivalent " +
			"using Go's strings.ToLower (Unicode-aware).",
		Examples: []string{
			`"HELLO" lower        => "hello"`,
			`"Café" lower         => "café"`,
		},
	...
	register(&Entry{
		...
		Notes: []string{
			"When chars is set with cs: \"insensitive\", both cases of each character are trimmed.",
			"Options: side, chars, cs, u, norm.",
		},
	})
	...
	register(&Entry{
		...
		Examples: []string{
			...
			`"cat" "c*t" {mode: "shell"} contains              => true`,
		},
		Notes: []string{
	...
	register(&Entry{
		...
		Notes: []string{
			...
			"Negative indices are Python-style: -1 means last character.",
