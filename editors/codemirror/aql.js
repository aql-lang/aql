// aql.js — a CodeMirror 6 language mode for AQL (the concatenative,
...
//
// It exports:
//   * aqlLanguage — a StreamLanguage instance (via StreamLanguage.define)
//   * aql()       — a convenience extension array [aqlLanguage, ...]
//
// This mode mirrors the vocabulary and faces of the reference Emacs mode
// (editors/emacs/aql-mode.el) and also drives the AQL web playground.
//
...
//
// Self-check: `node --check aql.js` — the only import is guarded below so the
...
// --- Vocabulary -----------------------------------------------------------
// Kept in sync with editors/emacs/aql-mode.el and the AQL lexical spec.
...
const parser = {
  ...
  startState() {
    return {
      ...
      // `${ ... }` embedded expression and tokenise AQL normally, tracking
...
// the embedded expression is tokenised as ordinary AQL code.
...
/**
 * The AQL language, as a CodeMirror StreamLanguage.
 * @type {import("@codemirror/language").StreamLanguage<unknown>}
 */
export const aqlLanguage = StreamLanguage.define(parser);

/**
 * Convenience extension: the AQL language plus room for companion extensions
