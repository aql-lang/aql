...
`aqleng/go/spec_test.go::renderValue`) goes through the
...
- **No tests for typed lists `[:Integer]` or typed maps
  ...
  Typed-collection sigs are covered in `aql/test/*.tsv` against
  the full parser.
...
- **No tests for the carrier / static-type-check mode**: the spec
  ...
  level tests in `aqleng/go/carrier*.go` and `aql/test/`.
...
(`aql/test/*.go` and `aql/test/*.tsv`); the aqleng spec subset is
...
All 396 rows pass under `go test ./aqleng/go/... -run TestSpec`.
