...
INSERT INTO bundle_meta VALUES ('input_digest_combined', '7994184594561182639');
...
INSERT INTO input_files VALUES ('../design/COMPILE-DECLARATION-MODEL.0.md', '1739108942267749589', 23570);
...
INSERT INTO input_files VALUES ('project/boru-project.jsonic', '7984021923111324928', 57887);
...
INSERT INTO entity_attributes VALUES ('ent:Document:801358522423407770', 'role', 'the proposal for what a word must tell the bytecode recorder, measured rather than argued: boru already has TWO general solutions — interpreter islands (OpFallback), built and then ratcheted to islandCeiling 0, and a CompileEffect taxonomy that grew to fifteen flags to replace them — and the trade between them has drifted past its optimum. The measurement: declaration sites per flag (CompileExecutesBody has ZERO — a dead member of a vocabulary that only grows), 153 ledgered frontier rows of which ~43 are fn-value shapes and ~36 provenance, frontier-hof-audit.tsv alone 65 rows or 43% of the ledger, and SEVEN rows that compile and answer correctly but are ledgered as failures purely because the program embeds an island. The diagnosis: CompileDefault = 0 is not an absence of information but the substantive claim that the handler re-steps fn operands on the tape, false for most words that receive one, and the flags are a patch set withdrawing it one word at a time — the same defect three times (var, service/add, and boru:fn-util, found 2026-08-25, where the correct declaration was a transcription of what invokeFnUtil visibly does). The proposal: collapse the ten operand-facing flags to three orthogonal per-position facts (tapeBound, needs a representation, env where names resolve — the last already reasoned about at emit.go:4827 with no field to hold it), and give OpFallback a declared RESULT CONTRACT so an escape stops poisoning the downstream type chain and the all-or-nothing cliff becomes a local step. What it does not fix: the provenance family, and the differential gate as a bug detector — NUR101 was found BY the two lanes disagreeing, with the compiled lane correct');
