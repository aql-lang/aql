/-
  ...
  `boru` engine by the differential harness in `harness/` — that is the
  ...
  `boru do 'add true 2'` yields `"2true"` and `'not 5'` yields `false`,
...
/-- Engine prints the stack bottom-to-top, space separated; errors as a
    sentinel. The harness compares this string to `boru do`'s output. -/
def render : Option Stack → String
  | none   => "ERROR"
  | some s => String.intercalate " " (s.reverse.map renderVal)


/-! ## 7. Executable sanity (mirrors `boru do '...'`) -/
