...
// assembler `boru do` and `boru test` go through, so pinning it here covers
...
func TestOptionsForCarriesStreamProbe(t *testing.T) {
	o := OptionsFor("", 0, nil)
	if o.Streams == nil {
		t.Fatal("OptionsFor carries no StreamProbe: boru do / boru test would answer IO.is-tty false on a real terminal")
	}
