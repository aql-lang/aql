...
type boruBridge struct {
	mu  sync.Mutex
	ctl *tuiController
}

func newBoruBridge(ctl *tuiController) *boruBridge { return &boruBridge{ctl: ctl} }

// spec is the host seam handed to modules.RegisterHostVault.
func (b *boruBridge) spec() modules.VaultSpec {
...
// execPrefix mirrors rootModel.execPrefix for the recipes op: "boru
...
func (b *boruBridge) execPrefix() string {
	...
	return "boru vault " + strings.Join(fl, " ")
}

// do executes one bridge op against the controller. The op names are
// the boru:vault word names; params carry the lang-side-validated
// arguments, map-shaped args flattened by key.
func (b *boruBridge) do(op string, params map[string]any) (any, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	c := b.ctl
	switch op {
...
// boruTuiSource is the two-line program the launcher runs: import the
...
const boruTuiSource = `import "boru:vault-tui"  (VaultTui.run {dark: %v}) drop`
...
func runInteractiveBORU(homeDir string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if err != nil {
		fmt.Fprintf(stderr, "boru-tui: %s\n", err)
		return 1
	}
	reg := a.NativeRegistry()
	registerBoruTuiBackends(reg, ctl, stdin, stdout)
	if err := b8runSource(a, fmt.Sprintf(boruTuiSource, lipgloss.HasDarkBackground())); err != nil {
		fmt.Fprintf(stderr, "boru-tui: %s\n", err)
		return 1
	}
	return 0
...
// registerBoruTuiBackends wires the two host seams the app dispatches
...
func registerBoruTuiBackends(reg *native.Registry, ctl *tuiController, stdin io.Reader, stdout io.Writer) {
	_ = modules.RegisterHostTui(reg, b8termbackSpec(stdin, stdout))
	_ = modules.RegisterHostVault(reg, newBoruBridge(ctl).spec())
}
