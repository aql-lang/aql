...
// ADR-008 coverage for aqlbridge.go — the modules.VaultSpec adapter and
// the `aql vault -i --aql` launcher. Ops run against REAL temp vaults
...
// probeBackendsRegistered proves registerAqlTuiBackends ran before the
// program: a second registration of either backend must be refused as
// already registered.
func probeBackendsRegistered(a *lang.AQL) error {
...
func newTestBridge(t *testing.T) (*aqlBridge, string) {
	t.Helper()
	ctl, home := newTestController(t)
	return newAqlBridge(ctl), home
}

// mustDo runs one op and fails the test on error.
func mustDo(t *testing.T, b *aqlBridge, op string, params map[string]any) any {
	t.Helper()
...
func TestBridgeSessionOps(t *testing.T) {
	...
	b.ctl.suffix = "work"
	if got := b.execPrefix(); got != "aql vault --folder=/elsewhere/vaults --suffix=work" {
...
func TestBridgeSecretOps(t *testing.T) {
	...
	if len(recipes) < 3 || !strings.Contains(asMap(t, recipes[0])["cmd"].(string), "aql vault exec gh_token") {
...
// The launcher: --aql routes into runInteractiveAQL; a non-terminal is
...
func TestRunInteractiveAQLArms(t *testing.T) {
	_, home := newTestBridge(t)

	// --aql flag routes here; non-TTY stdin (not an *os.File) refuses
	var errb strings.Builder
	if code := runInteractive([]string{"--aql"}, home, strings.NewReader(""), &errb, &errb); code != 1 {
		t.Fatalf("--aql without a terminal = %d", code)
	}
	...
	origNew := b8langNew
	b8langNew = func(...lang.Options) (*lang.AQL, error) { return nil, fmt.Errorf("b8-boom-init") }
	var errInit strings.Builder
	if code := runInteractiveAQL(home, devnull, devnullW, &errInit); code != 1 ||
	...
	var gotSrc string
	b8runSource = func(a *lang.AQL, src string) error {
		gotSrc = src
		return probeBackendsRegistered(a)
	}
	var out strings.Builder
	if code := runInteractiveAQL(home, devnull, devnullW, &out); code != 0 {
		t.Fatalf("success arm = %d, %q", code, out.String())
	}
	if !strings.Contains(gotSrc, `import "aql:vault-tui"`) ||
		!strings.Contains(gotSrc, "VaultTui.run {dark:") ||
		!strings.Contains(gotSrc, ") drop") {
		t.Fatalf("program source = %q", gotSrc)
	}
	b8runSource = func(*lang.AQL, string) error { return fmt.Errorf("b8-boom-run") }
	var errRun strings.Builder
	if code := runInteractiveAQL(home, devnull, devnullW, &errRun); code != 1 ||
	...
	// location to the home ~/.aql
	b8runSource = func(a *lang.AQL, src string) error { return nil }
	emptyHome := t.TempDir()
	t.Setenv(EnvFolder, "")
	t.Setenv(EnvSuffix, "")
	var out2 strings.Builder
	if code := runInteractiveAQL(emptyHome, devnull, devnullW, &out2); code != 0 {
