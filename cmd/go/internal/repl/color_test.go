...
// The REPL's error rendering: a structured BoruError gets the ANSI
...
func TestRenderREPLError(t *testing.T) {
	ae := native.MakeBoruError("signature_error", "boom detail", "w", "w x", "")
	...
	if got := renderREPLError(plain, true); got != "plain failure" {
		t.Errorf("non-BoruError must keep its text even with color on: %q", got)
	}
}
