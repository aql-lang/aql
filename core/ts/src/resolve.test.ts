...
describe('resolveWordValue', () => {
  ...
  it('lets a registry binding win over the builtin table', () => {
    ...
    assert.equal(resolveWordValue(newWord('Integer'), r).asString(), 'shadowed')
...
describe('resolveWordsDeep', () => {
  it('resolves list elements', () => {
    const out = resolveWordsDeep(newList([newWord('true'), newWord('Integer')]))
  ...
  it('flattens a nested disjunct and drops duplicate alternatives', () => {
    ...
    const inner = newDisjunct([newTypeLiteral(TInteger), newTypeLiteral(TString)])
