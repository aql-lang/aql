...
describe('makeConvert', () => {
  ...
  it('converts to Atom, and refuses an unsupported target', () => {
    ...
    assert.throws(
      ...
      (e: unknown) => e instanceof BoruError && e.message.includes('unsupported target type'),
    )
  })
})
...
describe('makePathon', () => {
  it('builds from a slash-separated string, collapsing runs', () => {
    assert.equal(makePathon(newString('a/b/c'), undefined)[0]!.toString(), 'a/b/c')
    assert.equal(makePathon(newString('a//b'), undefined)[0]!.toString(), 'a/b')
  })
...
describe('makeScalarHandler', () => {
  ...
  it('routes the three structured scalars to their builders', () => {
    assert.equal(
      makeScalarHandler([newTypeLiteral(TPathon), newString('a/b')])[0]!.toString(),
      'a/b',
    )
    assert.match(
      makeScalarHandler([newTypeLiteral(TEmailon), newString('u@h.com')])[0]!.toString(),
      /u@h\.com/,
    )
    assert.match(
      makeScalarHandler([newTypeLiteral(TUrlon), newString('https://h.com')])[0]!.toString(),
      /h\.com/,
    )
  })

  it('refuses a non-type-literal target', () => {
    assert.throws(() => makeScalarHandler([newInteger(1n), newInteger(2n)]), BoruError)
  })
})
...
describe('makeScalarOptsHandler', () => {
  it('reads abs from the options map for a Path target', () => {
    ...
    const out = makeScalarOptsHandler([newTypeLiteral(TPathon), opts, newString('a/b')])
  ...
  it('ignores a non-boolean or missing abs', () => {
    const empty = map([])
    assert.equal(
      makeScalarOptsHandler([newTypeLiteral(TPathon), empty, newString('a/b')])[0]!.toString(),
      'a/b',
    )
  })
  ...
  it('ignores the options entirely for a non-Path target', () => {
    const opts = map([['abs', newBoolean(true)]])
    const out = makeScalarOptsHandler([newTypeLiteral(TString), opts, newInteger(3n)])
...
describe('makeIdealHandler', () => {
  it('builds an Options instance from a source map', () => {
    ...
    const out = makeIdealHandler([newTypeLiteral(newType('Ideal/Options')), src])
