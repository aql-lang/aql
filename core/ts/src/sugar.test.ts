...
const render = (vs: ReturnType<typeof sugarExpansion>) =>
  vs.map((v) => v.toString()).join(" ");
...
describe("sugarExpansion — the roles that carry data", () => {
  it("lowers force-arity to its word plus the arity", () => {
    const out = sugarExpansion(
      boundRegistry(),
      { kind: "force-arity", n: 3n } as SugarInfo,
      false,
    );
...
describe("sugarExpansion — refusal", () => {
  it("raises sugar_unbound when the registry binds no word for the role", () => {
    ...
    assert.throws(
      ...
      (e: unknown) =>
        e instanceof BoruError && e.message.includes('sugar role "lambda"'),
    );
  });
  ...
  it("raises sugar_unbound for an unknown kind", () => {
    assert.throws(
      () =>
        sugarExpansion(
          boundRegistry(),
          { kind: "not-a-role" } as unknown as SugarInfo,
          false,
        ),
      (e: unknown) =>
        e instanceof BoruError && e.message.includes("unknown sugar kind"),
    );
  });
  ...
  it("refuses a role the registry binds only partially", () => {
    ...
    assert.throws(
      () =>
        sugarExpansion(
          r,
          { kind: "angle", name: "Box", items: [] } as SugarInfo,
          false,
        ),
      BoruError,
    );
...
describe("sugarExpansion — the lowered run is well formed", () => {
  it("produces values that render, not [object Object]", () => {
    ...
    for (const info of [
      ...
      assert.doesNotMatch(
        render(sugarExpansion(r, info, false)),
        /\[object Object\]/,
      );
    }
  });
