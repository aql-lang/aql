...
describe('renderSugar — one arm per marker kind', () => {
  it('renders the data-carrying kinds with their payload', () => {
    assert.equal(renderSugar({ kind: 'force-arity', n: 3n } as SugarInfo), 'sugar(force-arity 3)')
    assert.equal(
      renderSugar({ kind: 'mini', name: 'm', src: '1 add' } as SugarInfo),
      "sugar(mini m '1 add')",
    )
    assert.equal(
      renderSugar({ kind: 'angle', name: 'Box', items: [newWord('T')] } as SugarInfo),
  ...
  it('is what a Sugar VALUE renders as', () => {
    assert.equal(newSugar({ kind: 'lambda' } as SugarInfo).toString(), 'sugar(lambda)')
  })
})
...
describe('renderInterpSegments', () => {
  ...
  it('renders an expression-only interpolation', () => {
    assert.equal(renderInterpSegments([{ expr: [newInteger(1n)] }]), 'interp(${1})')
  })
...
describe('Value.toString — the marker arms', () => {
  ...
  it('renders a paren expression with its contents', () => {
    assert.equal(newParenExpr([newInteger(1n), newWord('w')]).toString(), 'paren([1 word(w)])')
  })

  it('renders containers', () => {
    assert.equal(newList([newInteger(1n), newString('s')]).toString(), "[1 's']")
    const m = new OrderedMap()
    m.set('a', newInteger(1n))
    assert.equal(newMap(m).toString(), '{a:1}')
    assert.equal(newTypedList(newTypeLiteral(TInteger), [newInteger(1n)]).toString(), '[:Integer 1]')
    assert.equal(newTypedMap(newTypeLiteral(TString), []).toString(), '{:String}')
  })
...
describe('canonValue tails', () => {
  ...
  it('renders an inspect value from its map', () => {
    assert.doesNotMatch(canonValue(newInspect(inspectMap())), /\[object Object\]/)
  })

  it('renders a disjunction with atoms bare and type literals by leaf', () => {
    assert.equal(
      canonValue(newDisjunct([newTypeLiteral(TInteger), newTypeLiteral(TString)])),
      'Integer tor String',
    )
    assert.equal(canonValue(newDisjunct([newAtom('a'), newAtom('b')])), 'a tor b')
  })

  it('joins a whole stack with canon', () => {
    assert.equal(canon([newInteger(1n), newString('s'), newNone()]), "1 's' none")
    assert.equal(canon([]), '')
  })
})
...
describe('canonXml', () => {
  it('renders a self-closing element when there are no children', () => {
    assert.equal(canonValue(newXml({ tag: 'br', attrs: [], children: [] })), '<br/>')
  })
  it('renders attributes and text children', () => {
    const x = newXml({ tag: 'a', attrs: [{ name: 'h', value: 'u' }], children: ['t'] })
