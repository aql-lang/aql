...
describe('inactive AnalysisImpl (a core build with no check piece linked)', () => {
  ...
  test('installAnalysisImpl replaces only the slots it is given', () => {
    ...
    assert.equal(AnalysisImpl.carrierResults, before, 'an omitted slot is left alone')
...
describe('inactive EmitRecorder (a core build with no compiler piece linked)', () => {
  ...
  test('every passive hook is a silent no-op', () => {
    ...
    assert.equal(inactiveEmitRecorder.recordValueIsland(v, v, 'desc'), undefined)
    assert.equal(inactiveEmitRecorder.alias(v, v), undefined)
    assert.equal(
      inactiveEmitRecorder.recordCall('w', {} as unknown as Signature, [v], [v]),
      undefined,
    )
  })
  ...
  test('every query declines', () => {
    ...
    assert.equal(inactiveEmitRecorder.constValueOf({ kind: 'const', value: v }), null)
  ...
  test('the constructing hooks throw rather than fabricate a value', () => {
    ...
    assert.throws(() => inactiveEmitRecorder.recordMakeList([]), /no recorder installed/)
    assert.throws(() => inactiveEmitRecorder.recordMakeMap([], []), /no recorder installed/)
  })
})
