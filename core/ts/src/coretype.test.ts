...
describe('parentType', () => {
  it('drops the last segment of a multi-part path', () => {
    ...
    assert.deepEqual(parentType(new BoruType(['Scalar', 'Number']))?.parts, ['Scalar'])
  })
  ...
  it('saturates the degenerate roots — they have no parent', () => {
    for (const name of ['Any', 'None', 'Never', 'Absent']) {
      assert.equal(parentType(new BoruType([name])), null, `${name} should saturate`)
    }
  })
})
...
describe('isTypeBody', () => {
  it('accepts typed containers and disjunctions', () => {
    ...
    assert.equal(
      isTypeBody(newDisjunct([newTypeLiteral(TInteger), newTypeLiteral(TString)])),
      true,
    )
  })
...
describe('isValueOfType', () => {
  it('accepts a value under a bare type literal by lattice subtyping', () => {
    ...
    assert.equal(isValueOfType(newString('x'), newTypeLiteral(TInteger)), false)
  })

  it('accepts a type value under the bare metatype Type', () => {
    assert.equal(isValueOfType(newTypeLiteral(TInteger), newTypeLiteral(TType)), true)
  })
  ...
  it('checks a typed list element-wise', () => {
    ...
    assert.equal(isValueOfType(newList([newInteger(1n), newString('x')]), t), false)
  })
