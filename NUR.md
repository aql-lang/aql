...
the eighth and ninth increments); OPEN for a read inside a CLOSURE body
(`[1] each [j]`, `do [j]`), for a declined point (a literal or a read
pushed before the statement and still pending at the test: `5  j typeof`)
...
error, a fn's effects once (TestBodyLocalDeoptParity, 61 rows). STILL
OPEN: a read inside a closure body — `[1] each [j]` ([42] / [fn]), `do
[j]` (42 / fn) — where the read lives in the closure unit and its frame
is the driving handler's; a point whose statement the compiled stack
