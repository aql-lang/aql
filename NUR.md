...
actually needs is a signal neither lane has: the engine notes only
fn-admitting bare reads (`noteWordRead`'s gate — `IsFnTypedCarrier` or a
gradual carrier admitting Function), while the tuple needs EVERY bare read
marked whatever its type. That is a kernel seam, so the increment records
rather than fixes. Pinned in lang/go/dyn_apply_head_name_test.go
