// Shared check-mode spec fixtures + runner for the TypeScript engine.
//
// Extracted from check.test.ts so both the check parity test and the
// cross-engine differential dumper (crossdump.ts) drive the checker through
// the IDENTICAL fixtures and rendering — no drift between the two.
//
// Runs the SAME corpus the Go engine-level check runner uses
// (eng/spec/check/*.tsv) and renders each row to the identical string (see
// renderCheck / test/go/engspec/engcheck_test.go::renderCheck). The TS engine
// must match the Go checker row-for-row.
//
// The fixtures here are a focused, return-annotated subset mirroring the
// engspec kernel fixtures the corpus exercises. In check mode the handlers
// are short-circuited (carrierResults synthesises the return carriers), so the
// handler bodies need only be present, not correct.
...
export const CHECK_SPEC_DIR = path.resolve(__dirname, '..', '..', 'spec', 'check')
...
export function runCheckRow(input: string): string {
...
export function parseCheckSpec(file: string): CheckRow[] {
