 ...
 */
export function sugarExpansion(r: Registry, info: SugarInfo, headForm: boolean): Value[] {
  switch (info.kind) {
    case 'usurp':
    case 'stack-args':
    case 'forward-args':
    ...
    case 'mini':
      return [sugarBoundWord(r, info.kind), newWord(info.name ?? ''), newString(info.src ?? '')]
    case 'type-bound':
      return [
        newParenExpr([newTypeLiteral(TType), sugarBoundWord(r, 'gen-apply'), ...(info.items ?? [])]),
      ]
    case 'angle': {
  ...
  throw new BoruError('sugar_unbound', `unknown sugar kind "${String(info.kind)}"`, String(info.kind))
}
