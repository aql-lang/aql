...
func TestResolveWordsDeepPreservesTypedContainerPayload(t *testing.T) {
	...
	// Boolean).
	tl := NewTypedListWithElements(NewWord("Integer"), []Value{
		NewInteger(1), NewWord("true"),
	})
	got := ResolveWordsDeepR(tl, r)
	ct, err := AsChildType(got)
	if err != nil {
		t.Fatalf("typed list lost its ChildTypeInfo: %v", err)
	}
	if !IsTypeLiteral(ct.Child) || !ValueType(ct.Child).Equal(TInteger) {
	...
	if b, berr := AsBoolean(ct.Elements[1]); berr != nil || !b {
	...
	tm := NewTypedMapWithEntries(NewWord("Integer"), []ChildEntry{
		{Key: "a", Value: NewInteger(1)},
		{Key: "b", Value: NewWord("false")},
	})
	got = ResolveWordsDeepR(tm, r)
	ct, err = AsChildType(got)
	if err != nil {
	...
	if b, berr := AsBoolean(ct.Entries[1].Value); berr != nil || b {
	...
	el := ResolveWordsDeepR(NewTypedList(NewWord("Integer")), r)
	if ct, err = AsChildType(el); err != nil || len(ct.Elements) != 0 {
		t.Errorf("empty typed list gained elements: %s", el)
	}
	em := ResolveWordsDeepR(NewTypedMap(NewWord("Integer")), r)
	if ct, err = AsChildType(em); err != nil || len(ct.Entries) != 0 {
