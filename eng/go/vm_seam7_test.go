...
func TestSeam7CheckReturnContractUnderflow(t *testing.T) {
	...
	err := checkReturnContract(r, fn, nil, 0, false)
	wantErr(t, err, "g")
	if err == nil {
...
// --- CALL_NATIVE handler returning a tape-coupled token (screenResults) ---

// seam7UserInc returns an appliable, non-delegation user-fn VALUE whose body
// calls the succeeding `cinc` native (a named param disqualifies the
// trivial-delegation fast path, so it islands).
func seam7UserInc(r *Registry) Value {
	return NewFnDef(FnDefInfo{
		Name: "cuserinc", Registry: r,
		Signatures: []Signature{{
			Params:  []FnParam{{Name: "n", Type: TInteger}},
			Returns: []*Type{TInteger}, BarrierPos: BarrierAllForward,
			Impl: AQL([]Value{NewWord("cinc"), NewWord("n")}),
		}},
	})
}

// TestSeam7ScreenResultsSeam swaps the screenResultsFn seam to force a
// rejection at each island / handler / poly dispatch site, then drives a
// SUCCESSFUL apply down every one of those paths so the otherwise-unreachable
// `return nil, err` screen arm executes. The tape-coupled token these arms
// defend against cannot be produced by a compiled-reachable handler or
// surfaced through an interpreter island, so the seam is the only way to
// exercise them (design/TEST-SEAMS.10.md).
func TestSeam7ScreenResultsSeam(t *testing.T) {
	r, _, _ := seam7DelegReg(t)
	inc := seam7UserInc(r)
	vc := seam7VC(r)

	old := screenResultsFn
	t.Cleanup(func() { screenResultsFn = old })
	screenResultsFn = func(_ *vmContext, _ []Value, label string, _ []SrcPos, _ int) error {
		return makeAqlError("internal_error", "forced screen rejection: "+label, "", "", "")
	}

	// callPoly (poly-dispatch result screen).
	if _, err := vc.callPoly(&PolyRef{Word: "cinc", Arity: 1}, []Value{NewInteger(5)}, seam7Dbg, 0); err == nil {
		t.Error("callPoly screen arm not taken")
	}
	// callDynamic island result screen (leading fn value).
	if _, err := vc.callDynamic(1, false, []Value{inc, NewInteger(5)}, seam7Dbg, 0); err == nil {
		t.Error("callDynamic island screen arm not taken")
	}
	// callDynTrailTop island result screen.
	if _, err := vc.callDynTrailTop(1, []Value{NewInteger(5), inc}, seam7Dbg, 0); err == nil {
		t.Error("callDynTrailTop island screen arm not taken")
	}
	// callDynApplyTop island result screen.
	if _, err := vc.callDynApplyTop(1, []Value{NewInteger(5), inc}, seam7Dbg, 0); err == nil {
		t.Error("callDynApplyTop island screen arm not taken")
	}
	// callDynMethod guard result screen (also drives the island `return guard`).
	if _, err := vc.callDynMethod(&DynMethodSpec{Word: "cuserinc", NArgs: 1, NOut: 1}, []Value{NewInteger(5), inc}, seam7Dbg, 0); err == nil {
		t.Error("callDynMethod guard screen arm not taken")
	}
	// callDynamicMixed island result screen (a window that computes cleanly).
	if _, err := vc.callDynamicMixed(2, []Value{NewWord("cinc"), NewInteger(5)}, seam7Dbg, 0); err == nil {
		t.Error("callDynamicMixed island screen arm not taken")
	}
	// runFallback island result screen.
	if _, err := vc.runFallback(&FallbackSpan{NIn: 0, Tokens: []Value{NewInteger(5)}, Desc: "fb"}, nil, seam7Dbg, 0); err == nil {
		t.Error("runFallback island screen arm not taken")
	}
}

// TestSeam7BindTypedScreenSeam drives OpBindTyped through run() with a
// SUCCESSFUL predicate bind while the screen seam forces a rejection, taking
// the typed-bind result screen arm in the main dispatch loop.
func TestSeam7BindTypedScreenSeam(t *testing.T) {
	r := seam7Reg(t)
	r.RegisterNativeFunc(NativeFunc{
		Name: "cistrue",
		Signatures: []Signature{{
			Args:    []*Type{TInteger},
			Returns: []*Type{TBoolean}, BarrierPos: BarrierAllForward,
	...
	pred := NewFnDef(FnDefInfo{
		Name: "cispos", Registry: r,
		Signatures: []Signature{{
			Params:  []FnParam{{Name: "n", Type: TInteger}},
			Returns: []*Type{TBoolean}, BarrierPos: BarrierAllForward,
			Impl: AQL([]Value{NewWord("cistrue"), NewWord("n")}),
		}},
	})
	...
	p := &Program{
		...
		Debug:      []SrcPos{{}, {}},
		TypedBinds: []TypedBindSpec{{Kind: TypedBindPredicate, Name: "x", Describe: "P", Cons: &pred}},
	}
	if _, err := RunProgram(p, r); err == nil {
		t.Error("BIND_TYPED screen arm not taken")
	}
}
