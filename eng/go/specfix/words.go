...
func registerEngSpecDefinition(r *eng.Registry) {
	...
	defFn := func(args []eng.Value, _ map[string]eng.Value, _ []eng.Value, reg *eng.Registry) ([]eng.Value, error) {
		...
		if !eng.IsConcrete(args[2]) { //covergate:allow raw NoEval List slot: the matcher only binds a parsed list literal here, which is always concrete
	...
	r.RegisterNativeFunc(eng.NativeFunc{
		...
		Signatures: []eng.Signature{{
			...
			Impl: eng.Go(func(args []eng.Value, _ map[string]eng.Value, _ []eng.Value, reg *eng.Registry) ([]eng.Value, error) {
				if !eng.IsConcrete(args[0]) { //covergate:allow raw NoEval List slot: the matcher only binds a parsed list literal here, which is always concrete
...
func registerEngSpecStack(r *eng.Registry) {
	...
	op := func(name string, argCount int, fn func(args []eng.Value) []eng.Value, identity ...int) {
		...
		if len(identity) == 0 {
			sig.Returns = []*eng.Type{}
		} else {
			sig.ReturnsFn = eng.ReturnsIdentity(identity...)
		}
		r.RegisterNativeFunc(eng.NativeFunc{Name: name, Signatures: []eng.Signature{sig}})
	}
	op("dup", 1, func(a []eng.Value) []eng.Value { return []eng.Value{a[0], a[0]} }, 0, 0)
	op("drop", 1, func(_ []eng.Value) []eng.Value { return nil })
	op("swap", 2, func(a []eng.Value) []eng.Value { return []eng.Value{a[0], a[1]} }, 0, 1)
	op("over", 2, func(a []eng.Value) []eng.Value { return []eng.Value{a[1], a[0], a[1]} }, 1, 0, 1)
	op("rot", 3, func(a []eng.Value) []eng.Value { return []eng.Value{a[1], a[0], a[2]} }, 1, 0, 2)
	op("nip", 2, func(a []eng.Value) []eng.Value { return []eng.Value{a[0]} }, 0)
	op("tuck", 2, func(a []eng.Value) []eng.Value { return []eng.Value{a[0], a[1], a[0]} }, 0, 1, 0)
	op("dup2", 2, func(a []eng.Value) []eng.Value { return []eng.Value{a[1], a[0], a[1], a[0]} }, 1, 0, 1, 0)
	op("swap2", 4, func(a []eng.Value) []eng.Value { return []eng.Value{a[1], a[0], a[3], a[2]} }, 1, 0, 3, 2)
	op("drop2", 2, func(_ []eng.Value) []eng.Value { return nil })
	op("over2", 4, func(a []eng.Value) []eng.Value { return []eng.Value{a[3], a[2], a[1], a[0], a[3], a[2]} }, 3, 2, 1, 0, 3, 2)
}
...
func registerEngSpecTypeWords(r *eng.Registry) {
	...
	r.RegisterNativeFunc(eng.NativeFunc{
		...
		Signatures: []eng.Signature{{
			...
			Impl: eng.Go(func(args []eng.Value, _ map[string]eng.Value, _ []eng.Value, _ *eng.Registry) ([]eng.Value, error) {
				list := args[0]
				if !eng.IsConcrete(list) { //covergate:allow raw NoEval List slot: the matcher only binds a parsed list literal here, which is always concrete
...
func registerEngSpecStorage(r *eng.Registry) {
	setObjectH := func(args []eng.Value, _ map[string]eng.Value, _ []eng.Value, _ *eng.Registry) ([]eng.Value, error) {
		container := args[2]
		if !eng.IsConcrete(container) { //covergate:allow runtime-only handler: dispatch never binds a bare type node into a value slot and runtime values are never carriers; mirrors the production storage word where check mode can reach this
...
func registerEngSpecObjectRecord(r *eng.Registry) {
	recordH := func(args []eng.Value, _ map[string]eng.Value, _ []eng.Value, r *eng.Registry) ([]eng.Value, error) {
		list := args[0]
		if !list.Parent.Equal(eng.TList) { //covergate:allow sole caller refineCtorH pre-checks Parent==List; kept to mirror the production record word, which is dispatched directly
	...
	objectWithParentH := func(args []eng.Value, _ map[string]eng.Value, _ []eng.Value, r *eng.Registry) ([]eng.Value, error) {
		...
		if !eng.IsClassType(parentVal) { //covergate:allow sole caller refineCtorH pre-checks IsClassType(base); kept to mirror the production object word, which is dispatched directly
