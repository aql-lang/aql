...
func TestForkConcurrentIsolation(t *testing.T) {
	...
	r.Defs.Push("shared", NewInteger(1))
	...
	for i := 0; i < n; i++ {
		f := r.ForkConcurrent()
		go func(idx int, reg *Registry) {
			defer wg.Done()
			// Each fork writes a distinct private binding and a context key.
			reg.Defs.Push("local", NewInteger(int64(idx)))
...
func TestForkConcurrentInheritsDefsSnapshot(t *testing.T) {
	...
	r.Defs.Push("x", NewInteger(99))
	if v, _ := f.Defs.Top("x"); func() int64 { n, _ := AsInteger(v); return n }() != 7 {
