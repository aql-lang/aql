...
// MemberUnifier.Equal are plain kernel-default delegates.
func TestDisplacedBehaviorMatchArms(t *testing.T) {
	...
	mb := MemberBehavior(func(v Value) bool { return true }).(MemberUnifier)
	if !mb.Equal(NewInteger(1), NewInteger(1)) || mb.Equal(NewInteger(1), NewInteger(2)) {
		t.Fatal("MemberUnifier.Equal delegates to the kernel default")
	}
}
