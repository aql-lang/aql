...
func TestW8SpillSeatUnderflow(t *testing.T) {
	...
	if reason := lw.spillSeat([]emitOperand{constOperand(0)}, []int{0}, 1, SrcPos{}, "SPILLFAIL"); reason != "SPILLFAIL" {
...
func TestW8SpillSeatNonOperandInterleaved(t *testing.T) {
	...
	reason := lw.spillSeat([]emitOperand{eventOperand(1, 0)}, []int{0}, 1, SrcPos{}, "SPILLFAIL")
	if reason != "SPILLFAIL" {
...
func TestW8LayoutOperandsReverseMissSpills(t *testing.T) {
	...
	if reason := lw.layoutOperands(ops, SrcPos{}, msg); reason != "SHAPEBEYOND" {
...
func TestW8LayoutOperandsResultNotTop(t *testing.T) {
	...
	if reason := lw.layoutOperands(ops, SrcPos{}, msg); reason != "RESULTNOTTOP" {
...
func TestW8SeatResultsAboveLiteral(t *testing.T) {
	...
	if reason := lw.seatResults(ops, false, false, w8seatMsgs(), SrcPos{}); reason != "ABOVELIT" {
...
func TestW8SeatResultsReordered(t *testing.T) {
	...
	if reason := lw.seatResults(ops, false, false, w8seatMsgs(), SrcPos{}); reason != "REORDERED" {
...
func TestW8SeatResultsUnconsumed(t *testing.T) {
	...
	if reason := lw.seatResults(nil, false, false, w8seatMsgs(), SrcPos{}); reason != "UNCONSUMED" {
...
func TestW8LowerFragmentDepthLimit(t *testing.T) {
	...
	if reason := lw.lowerFragment(&EmitFragment{}, nil, false, SrcPos{}); !strings.Contains(reason, "beyond the compile depth limit") {
...
func TestW8LowerFragmentApplyDiverges(t *testing.T) {
	...
	if reason := lw.lowerFragment(frag, nil, false, SrcPos{}); reason != "" {
...
func TestW8LowerFragmentApplyNotSole(t *testing.T) {
	...
	if reason := lw.lowerFragment(frag, nil, false, SrcPos{}); !strings.Contains(reason, "leading fn value not the sole residual") {
...
func TestW8LowerFragmentOutNilExtra(t *testing.T) {
	...
	if reason := lw.lowerFragment(frag, nil, false, SrcPos{}); !strings.Contains(reason, "body leaves extra values") {
...
func TestW8LowerFragmentMultiValueNoVariadic(t *testing.T) {
	...
	if reason := lw.lowerFragment(frag, &out, false, SrcPos{}); !strings.Contains(reason, "branch leaves extra values") {
...
func TestW8LowerFragmentLeftoverDrop(t *testing.T) {
	...
	out := constOperand(0)
	frag := &EmitFragment{events: []emitEvent{w8pushEvent(5, 1)}}
	if reason := lw.lowerFragment(frag, &out, true, SrcPos{}); reason != "" {
...
func TestW8TailCompatibleReturns(t *testing.T) {
	...
	if es.tailCompatibleReturns(-1, []*Type{TInteger}) {
		t.Fatal("out-of-range callee unit should be incompatible")
	}
	// callee return does not conform to the caller's declared return.
	es.fnRecs = []*fnUnitRec{{returns: []*Type{TString}}}
	if es.tailCompatibleReturns(0, []*Type{TInteger}) {
...
func TestW8LowerComputedBranchThenComputedElseRefusal(t *testing.T) {
	lw := w8lw()
	lw.emit(OpJmpIfFalse, 0, SrcPos{}) // pre-emit so jf=0 is a valid patch target
