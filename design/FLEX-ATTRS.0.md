...
2. **Only the PARSER sorts.** Map literals are converted by iterating
   ...
   (`"ck"`, `"qm"`, `"qk"`, `"sh"`), never surfaced to AQL.
...
|---|---|---|
| R | `attr()` — instance attributes that change behaviour (`class`, `dim`, `names`) | The cautionary tale. Propagation was never specified ("attributes are generally discarded" — R Language Definition); every pipeline stage silently drops some; `class`-as-attribute (factors!) is a permanent bug generator. AQL would replay the propagation debt on day one (§3.4) — except ADR-008 forces AQL to pay it explicitly, per boundary, forever. |
...
| Python | dict / weakref | Same convergence independently: 3.7 made insertion order universal (no mode; and order stays OUT of dict equality — matching AQL's `deq` today); weakness lives in separate types whose docs warn entries "may be discarded at any time". |
...
out of AQL's existing reference/value split — attrs are properties of
