...
`boru:id` is a utility module for identifiers, not secret bearer tokens. It uses `boru:crypto` when unpredictability is required, but its primary concerns are uniqueness, ordering, readability, interoperability and naming.
...
The module should define canonical separators, prefix validation and total-length calculation. Secret API tokens with prefixes remain in `boru:crypto`; ordinary entity IDs belong here.
...
`boru:id` consumes cryptographically secure randomness from `boru:crypto` for random profiles. It does not reimplement random number generation.
