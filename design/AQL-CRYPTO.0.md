...
> authoritative spec for the deferred **"vault logic in AQL"** phase of
...
> byte-for-byte, or an in-AQL vault cannot open a vault the Go path wrote.

## 1. Package & status

`aql:crypto` is the curated home for the cryptographic primitives the
vault needs to move its key-management and envelope logic out of Go and
into AQL: authenticated encryption, a password KDF, a sub-key KDF,
...
behind the `aql:vault` bridge. This module is step one of shrinking that
bridge — once `Crypto.*` exists, `store.jsonic` read/write, keyslot
envelopes, and capability bookkeeping become expressible in AQL (`aql:io`
already covers atomic writes, locks, and permissions), and the bridge
retires op by op.

## 2. Why curated — and the `aql:rand` firewall
...
- and keeps secure randomness **here**, never in `aql:rand`.

**The `aql:rand` firewall (CRITICAL).** `aql:rand` (`Rand`) is backed by
...
on `crypto/rand`. `aql:rand` must never be reached for anything a
...
```aql
import "aql:crypto"        # binds the Crypto namespace
```
...
`aql:bin-util` (base64/hex; the `Bytes`↔hex/base64 handoff is documented
...
Crypto.aead-open <key> <nonce> <aad> <sealed> → Bytes   # plaintext, or [aql/crypto] on failure
```
...
HKDF-SHA256; `info` is the domain-separation label (e.g. `"AQL-slot-kek"`),
...
password slot's public key (§5), so an in-AQL keyslot can unwrap existing
`WrappedKeys`.
...
| Sub-key KDF | `hkdf` | HKDF-SHA256, 32-byte output, info label e.g. `"AQL-slot-kek"` | `keyslot.go` `slotKEK` (`hkdf.New(sha256.New, kek, salt, label)`) |
...
AQL already has a **first-class `Bytes` type** — `Scalar/Bytes`, FixedID
...
`List[Integer]` (BYTES.10 §1). `aql:crypto` uses it directly: every key,
...
- **`convert` / `slice` / `add` / `size`** for framing envelopes, and a
  ...
  `BinarySpec`) that the AQL layer can use to build the on-disk envelope
...
4. A `checkCryptoPolicy(r, op)` gate copying `checkVaultPolicy`
   ...
   allow; `!pol.Installed("crypto")` → `[aql/capability_not_installed]`;
...
§7.2 of the RFC, deliberately homed in `Crypto` and **not** `aql:rand`
(§2 firewall).
...
- **`aql:rand` (`Rand`)** — `math/rand`, seedable, reproducible;
...
- **`aql:bin-util` (`BinUtil`)** — the boundary codecs (`base64-*`,
...
- **`aql:vault` (`Vault`)** — the consumer. Today the bridge does the
  crypto in Go; the migration reverses that so the in-AQL vault calls
  `Crypto.*`.
- **`aql:io` (`IO`)** — atomic writes, file locks, and permissions the
  in-AQL vault uses to persist the sealed envelopes `Crypto.*` produces.
...
- **Module.** `BuildCryptoModule(parent)` in `lang/go/modules/crypto.go`,
  ...
  resolver (`InstallResolver`) and add `moduleDocs["aql:crypto"]`.
...
- **Governance gates.** Add the module to `help.ModuleCatalog`
  ...
  `check-accuracy` ratchet — the same gates the `aql:vault` module had to
  satisfy.
