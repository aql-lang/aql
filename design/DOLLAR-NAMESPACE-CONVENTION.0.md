...
AQL would benefit from a language-wide visual convention that tells a developer, immediately and consistently, that a name belongs to the language or runtime rather than to ordinary application code. Dollar-prefixed names are a plausible convention.
...
External data is allowed to contain keys beginning with `$`. JSON documents, database records and third-party protocols commonly use such names. AQL must preserve those keys exactly and permit ergonomic access to them.
