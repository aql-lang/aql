...
AQL is batteries-included. The crypto module should not merely reproduce Go's package structure or Node's low-level object APIs. It should expose a coherent AQL façade over host cryptography, combining precise primitives with common modern recipes.
...
Profiles may produce API keys, session tokens, reset tokens and other opaque identifiers. AQL ID may consume crypto randomness but semantic secret tokens remain crypto recipes.
...
For now, structured tokenization remains in `aql:crypto`, marked for reconsideration.
...
This area is broader than cryptography and may later become `aql:privacy` or another module. It must not be confused with execution context or identifier generation.

## 14. Constant-time behaviour

Do not offer a callback wrapper that claims to make arbitrary AQL code constant-time. The runtime, compiler, garbage collector, data-dependent allocations and called code make such a guarantee unsound.
...
- `aql:crypto` supplies all algorithms and recipes;
- Vault is primarily AQL business logic using `aql:crypto`;
...
A future `aql:trust` module should expose semantic operations such as user presence, user verification, credential creation and opaque hardware-bound key handles. Those handles flow into crypto signing/decryption APIs.
...
Opaque key handles and secret values require VM-safe lifetime and cleanup semantics. Native errors become AQL errors, never panics.
