...
`boru describe`. The value is the *shape* of the code, not its execution.
...
| --- | --- | --- |
| `todo.boru` | **service** | owned `Store` state, `add`/`call`/`send`, scalar-tag routing, `no_match`, event emission |
| `auth.boru` | **prior + wrap** | cross-cutting auth with zero handler edits; blanket gate (`wrap`) + per-pattern owner guard (`prior`) |
| `audit.boru` | **wrap** | ambient logging around the whole dispatch; observing app errors without swallowing them |
| `sessions.boru` | **service + process** | identity (service) + presence (one `spawn`ed process per user, `register`/`whereis`) |
| `presence.boru` | **process** | raw `spawn`/`receive` loop, two-layer clause matching, `after`, `self`, correlation tags |
| `notify.boru` | **service** | bounded lossy sink (`send`, `overflow: 'drop`), deferred reply (`defer` + `reply req.@from`) |
| `app.boru` | **server + transport** | `server`/`serve` with restart policy, `pool` hash-sharding, `listen` |
| `client.boru` | **connect** | the uniform failure contract: idempotent retry vs. reconcile, correlated push |

### Request lifecycle (a `create`)

```
client.boru                app.boru graph                         process layer
----------                -------------                         -------------
...
and binds `name:Type` slots. In `presence.boru` this is pleasant —
`{cmd: "ping" reply: Pid}` obviously means "route on `cmd:"ping"`, bind a typed
`reply`." But note the asymmetry the app exposes: **services** (`todo.boru`)
...
(`presence.boru`) get the `name:Type` sugar in `receive`. A newcomer will try
...
`client.boru` matches the `subscribe` reply with `{ok:Boolean user:String}` and
the push with `{push:String user:String}`, never a bare `{}`. `presence.boru`
...
**Verdict: confirmed, emphatically.** `todo.boru` + `auth.boru` + `audit.boru`
...
`sessions.boru` and `serve`/`listen` in `app.boru` are the only capability-gated
...
**Verdict: ergonomic.** `presence.boru`'s `(after 30000)` heartbeat and
`client.boru`'s `(after 60000)` drain-or-give-up read naturally as just another
...
`app.boru`. Recommend the RFC allow per-service mailbox/overflow overrides, with
...
- **Services are values; layers are functions over values.** The whole graph
  in `app.boru` is ordinary composition — `Audit.log (Auth.gate sessions (…))`.
  No registration, no annotations, no config format.
- **`prior`/`wrap` paid for itself immediately.** Auth and audit are entirely
  absent from `todo.boru`; they are added from the outside in `app.boru`. This is
  the design's clearest DX win.
- **The service↔process contrast is legible.** `todo.boru` (service) vs.
  `presence.boru` (raw process) makes the "request/reply with owned state" vs.
