# Seneca-style entities & store plugins on AQL — a DX probe

Illustrative, **non-executable** AQL (the SERVICES words and the `Entity` Ideal
type don't exist yet), answering the question: *how would Seneca's `seneca-entity`
data layer and its store plugins (e.g. `seneca-dynamo-store`) be built on AQL using
...
AQL's `SERVICES.0` model **is** Seneca's core, re-derived on the same vendored
patrun. The entity layer and store plugins are ordinary AQL services and modules
on top:
...
| **entity layer (make$/.save$)** | **`Entity` Ideal type + `aql:entity` ops** | this probe |
| **store plugin** | **a module exporting a store `Service`** | this probe |
| Seneca store `map` routing | **patrun specificity over canon tags** | `entity.aql` `mount` |
...
| File | Role |
| --- | --- |
| `entity.aql` | the entity **bus** (canon→store router) + the **handle ops** (`make`/`save`/`load`/`list`/`remove`) |
| `mem-store.aql` | the phase-1 in-memory store (`seneca-mem-store` analog) — pure `Store`, no I/O |
| `dynamo-store.aql` | a DynamoDB store **sketch** via `Net.fetch`, with explicit `# GAP:` prerequisites |
| `usage.aql` | the DX: two stores routed by canon, a timestamp + audit `wrap`, then `make`/`save`/`load`/`list`/`remove` |
| `store-test.aql` | the `seneca-store-test` conformance checklist every store must pass |

## How an entity op flows (a `save`)

```
usage.aql                         entity.aql                    a store module
---------                         ----------                    --------------
...
- **The engine question is settled by reuse.** `mem-store.aql` is a plain
...
- **The `Entity` Ideal type earns the `make$/.save$` feel** — but note this probe
  models the handle as a Map `{canon data bus}` because AQL can't mint a Go-level
  Ideal type from `.aql`. The real type's `Field` hook would make `u.name` read a
...
- **We dropped Seneca's `$`-suffix.** `list ent {team:"x"} {sort:{name:1} limit:2}`
  ...
  around. (See `store-test.aql` scenarios 4–5.)

- **`prior`/`wrap` give entity middleware with zero store changes.** `usage.aql`
...
- **The real store gaps are honest and concrete.** `dynamo-store.aql` shows the
  store *shape* is trivial, but flags the true blockers: **AWS SigV4 signing**
  (AQL has no HMAC/crypto/signer), the **`network`** capability scope, and the
  upsert race the real plugin documents. mem-store needs none of this — hence
  "mem-store first" (ENTITY-STORES §8).

- **One naming clash to resolve:** the op `list` collides with AQL's core `list`;
  here the ops are `aql:entity`-namespaced (`Entity.list`). The RFC's Open Q #1
  tracks whether to keep them namespaced or add object-method sugar (`e.save`).

## Cross-references

- Full design + the Seneca→AQL mapping: `../../ENTITY-STORES.0.md`
