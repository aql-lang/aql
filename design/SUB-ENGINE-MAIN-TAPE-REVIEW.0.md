...
5. One genuine sub-engine→main-tape flip is already on record as a
   residual: `execFnDefSig`'s CallAQL branch when
...
- **Error trap.** `sub.Run` returning an error is the interpreter's
  try/catch. `do`+`error`, the `aql:test` runner, help-example
...
- **Fn frames** (`fn_frame.go`): `(ₘ args… body… __DC __pa undef…
  ...
  `CallAQL` does in Go around its sub-engine.
...
- rework of check-mode analysis and the bytecode story: these words
  ...
  (`design/aql-bytecode-plan.0.md` P1); the interpreter and VM paths
...
|---|---|---|
| Named `def f fn […]` call | `CallAQL` sub-engine per call | body spliced as a main-tape frame; TCO elides tail frames (TCO-STAGED Stages 1–4b) |
| Module-fn self/mutual recursion | (believed Go-recursive) | one `CallAQL` boundary per entry, then main-tape frames inside (TCO-STAGED Stage 5 finding) |
...
| Module wrapper delegation | CallAQL per call | `isTrivialDelegationBody` short-circuit → direct `execMatch`, "no body splicing, no sub-engine" |
...
- **B-SEAM** — sits behind `InvokeBody`/`CallAQL` for VM parity;
...
|---|---|---|---|
| `registry.go:1467` `CallAQL` | module fns, host calls, fn values via captured registry | B-REG (module case), B-ISO, B-SEAM | **Keep.** Named same-registry fns already bypass it via frame splice; what remains is mostly cross-registry by construction. |
| `engine.go:4986` `execFnDefSig` CallAQL branch, `capturedReg == e.registry` | a module-fn value applied inside its own module (callbacks passed back in) | none hard — same registry, results already spliced back | **Convert (R2).** Already recorded as a TCO-STAGED Stage-5 residual: "routing it onto the splice branch is a small flip"; needs drain/flow boundary spec rows first. |
...
| `core_helpers.go:247` etc. | remaining CallAQL delegations | as CallAQL | **Keep.** |
...
**R2 — Flip `execFnDefSig`'s same-registry CallAQL branch to the
