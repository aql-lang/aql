# Local Reasoning for Global Properties in AQL: Applicability Report

## Scope

Evaluate AQL against the thesis of Laurence Tratt's *Local Reasoning for
...
1. To what extent does AQL's existing design already deliver "local
...
notes, plus **live probes against the built binary** (`aql 0.1.0-dev`,
git `d5bcd36`): paired `aql check -e` / `aql do` experiments, policy
...
- **static** — `aql check` (or def/parse time) rejects before execution;
...
[`effect-oriented-programming-in-aql-report.0.md`](effect-oriented-programming-in-aql-report.0.md),
...
| Family-restricted `lt`/`gt`/`cmp` (`[aql/incomparable]`) | No silent cross-family ordering anywhere | Call site | static for literals + runtime |
...
## What AQL already delivers

### 1. Signature-carried value contracts — the genuine Rust analogue

This is AQL's strongest instance of the article's pattern, and it is
...
`no_signature` at check time and `[aql/signature_error]` at runtime;
...
- **Predicate returns are runtime-only.**
  `def mkbad fn [[] [Big] [5]] mkbad` passes `aql check` clean (exit 0,
...
nearest thing AQL has to Rust's `T: Send` *shape*: a constraint on a
...
AQL's strongest genuinely *local* effect reasoning today is not in
...
`aql policy explain` answers "why was this denied?" deterministically.
...
- **Family-restricted comparison**: `1 lt "a"` raises
  `[aql/incomparable]` (Error-severity in check for literals) rather
...
knowledge into every signature; AQL concentrates it into **one ambient
...
move: look at a *word* and know what it may touch. `aql check` knows
...
- Two branches incrementing one outer FlexMap 300× each:
  **`fatal error: concurrent map writes`** — a Go-runtime process
  kill, exit 2, uncatchable by `do … error`. `aql check` passes the
...
So today AQL sits *below* the pre-Rust baseline the article describes:
...
The article's core symptom is unnecessary defensive checks. AQL
...
- The assertion-vs-defensive distinction is convention-only: there is
  no core `assert`; `Assert.*` (aql:test) and `Debug.assert`
  (aql:debug) raise `assertion_failure` as an *ordinary catchable
...
Some of AQL's foundations cut against the article's premise that local
...
AQL's versions must be **gradual with a runtime backstop**, never
...
- Bare `aql run` never checks; every static guarantee above holds only
  for users who run `aql check` (or `run --check`). Under the
...
- `aql check --effects` prints the program's inferred envelope;
- `aql check --perms <profile>` diffs envelope against policy
...
The Pony comparison matters here: AQL's existing taxonomy — immutable
...
- Define `Sendable` = immutable values + explicitly-snapshotted
  ...
  or reject with a coded error (`[aql/not_sendable]`) naming the value
...
This is the article's own showcase property, purchasable in AQL's
...
info. Symmetry is AQL's stated principle; today the static layer is
...
fact in AQL; only the top-level-literal case (`def ne:NEL []`, the
...
advisory: an open error world is settled AQL design; the point is
...
the monotone-attenuation story breaks. Migrate `Assert.*` (aql:test)
and `Debug.assert` (aql:debug) onto it. Then "this cannot happen" and
...
bounds are on the article's candidate list, and AQL has already done
...
Flip `aql run` to preflight-check with `--no-check` opt-out. This is
...
`aql describe` is the sanctioned ground truth for agents — this repo's
...
`aql check --profile agent` (or `--strict=agent`): promote the
...
- **No ownership/borrow system.** AQL's value taxonomy + fork
...
- **No soundness theater.** Dynamic resolution, `do` over data, and
  catch-all overloads mean AQL's guarantees are gradual with runtime
...
**AQL already practises the article's discipline at the value layer,
...
AQL local answers to "what can this call touch?", "can these branches
