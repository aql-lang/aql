...
large, real application entirely in AQL. These are findings *about the
...
It is the place to record "AQL made me do an unobvious thing" so the next
...
the first large application built on the `aql:tui` runtime
(`lang/go/modules/vault_tui.aql`, ~700 lines, ~80 functions). The port ran the
...
- **Latent bug:** this exact broken idiom ships in
  `design/examples/apps/todo-tui-client.aql` (its `error [ def why (dot
...
*does `vault_tui.aql` compile fully to bytecode without refusals?*
...
- **The launcher runs the program on the interpreter, unconditionally.**
  `cmd/go/internal/vault/aqlbridge.go:360,389` execute the two-line app source
  via `a.RunInterp(...)` — never `.Run` (compile-try) or `RunCompiled*`. Per
  `lang/go/aql.go:754`, `RunInterp` "executes on the tree-walking interpreter,
...
- **The module preamble is loaded on the interpreter.**
  ...
  no body is compiled at import (identical to `sift.aql`, `repl.aql`).
- **`Tui.run` dispatches every `update`/`view` callback through the
  ...
  the callback has a stamped `CompiledRef`; with none, it falls to `CallAQL` —
  the interpreter — on every keystroke and every frame.

So the app is interpreted exactly like every other AQL app in the repo. The
...
| G10 | `def why (dot message)` in `error` | latent-bug | fix `todo-tui-client.aql` + add a failing-sync test |
