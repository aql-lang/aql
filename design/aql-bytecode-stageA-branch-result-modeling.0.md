# AQL Bytecode — Stage A deep-dive: branch-result modeling (the multi-value / variadic arm)
...
and to the forward plan in `aql-bytecode-next-stages.0.md` **Stage A**. It picks
...
Read `aql-bytecode-next-stages.0.md` §Stage A first for the original framing;
...
## 2. Two corrections to `aql-bytecode-next-stages.0.md` §Stage A
...
- **Full gate:** `make fmt && make vet && make lint && make test`, then
  ...
  property fuzz + `-race` + `aqldebug`, **0** divergences) and `make status`.
