# `aql:codec` Representation Module
...
AQL already has ParseLang and built-in facilities for structured formats such as JSON and CSV. A new codec module must not duplicate that work.
...
- Unicode scalar/rune sequences where AQL has such a carrier;
