# Rust, Zig, Roc, Faber — Language Lessons Considered Against AQL

## Scope

This report answers two questions asked of the AQL project in July 2026:

1. Review Richard Feldman's article **"rust-to-zig"**
   (rtfeldman.com/rust-to-zig, the retrospective on rewriting the Roc
   compiler from Rust to Zig) and identify ideas AQL can reuse and
   pitfalls AQL can avoid.
2. Examine **Rust, Zig, Roc, and Faber** (faberlang.dev) *as languages*,
   in the same manner, compared to AQL.

It is a point-in-time analysis report in the lineage of
`LISP-ANALYSIS.5.md`, `RACKET-ANALYSIS.5.md`,
`elixir-types-in-aql-report.10.md`, and
`fsharp-units-in-aql-report.0.md`. **No code changes** are proposed as
decided here; every "idea" is a candidate for its own design note. AQL
...
AQL is not choosing between Rust and Zig — it is a Go interpreter
...
### 1.1 Where AQL already validates the article

- **"Miscompilations are the real risk."** AQL's own history says the
  ...
  fn-member auto-dispatch), zero memory bugs. And AQL already runs the
...
- **Data-oriented design, independently arrived at.** Roc restructured
  around SoA and indices; AQL did the same species of work in Go: the
...
- **Resource exhaustion as ordinary errors.** The article praises Zig
  treating allocation failure as a normal error. AQL's step budget
  (`[aql/evaluation_limit]`) and tape ceiling
  (`[aql/tape_exhausted]`) are the same move: limits are named,
...
- **The index-safety lesson, productized.** With u32 indices,
  ...
  *types*. That is AQL's bare-`refine` newtype
...
- **Subtractive design.** ADR-002 (no implicit broadcasting), ADR-005
  ...
  from `quote`/`unquote` rather than a second system: AQL's ADRs are
...
1. **Cache format follows data representation.** The planned module
   ...
   `InheritConfig` channel that doc describes. Honest caveat: `aql`
   startup is ~20ms — memcpy-grade loading is not the point at AQL's
...
4. **LLM-classified issue retrospectives as recurring practice.** The
   article classified ~3K issues to ground its safety claim; AQL
   already runs agent-driven hunts (the 30-agent miscompile sweep).
   Periodically classifying AQL's own issue history by root-cause
   class would show where the *next* verify-gate belongs.
5. **The 35ms lesson, applied to AQL's loop.** Go compiles fast; AQL's
   analog is user-facing latency. `aql check`/LSP redo work per
...
- **Pre-1.0 churn is a tax on users** — the Roc team's loudest
  complaint about Zig. AQL's ~11K-row executable spec is already a
...
- **A cache is not a transparent optimization.** Roc could zero-parse
  cache because caching was semantically invisible. AQL's module
...
- **Don't fight yesterday's safety war.** The differentiating
  ...
  covers it for AQL. The defect class that actually bites is engine
...
- **The rewrite lesson, inverted.** Roc rewrote because the
  architecture was wrong — after prototyping the fix in OCaml. AQL's
...
- Rust's newtype pattern (tuple structs) ↔ AQL's bare `refine`;
  REFINE-NEWTYPE-VS-SUBSET cites Rust explicitly, and AQL's version
  enforces one membership rule at parameters, returns, and `is`.
- Rust's integrated toolchain (cargo/rustfmt/clippy/rustdoc) ↔ AQL's
  single binary (`fmt`, `check`, `lsp`, `describe`, …). AQL's
  `describe` is stronger than rustdoc in one respect: output is
  generated from the live engine, so it cannot drift.
- Clippy's advisory-vs-error split ↔ `aql check`'s never-gating
...
- **Editions** — the standout. Rust ships breaking changes per-crate,
  opt-in, with ecosystem interop preserved. AQL analog: stamp spec
  rows and modules (via `aql.json`) with a language edition and let
...
- **Visible fallibility.** Rust's `?` marks every fallible call site.
  AQL is the inversion — open errors propagate implicitly; `do […]`
...
- **The upfront-complexity cliff.** Rust makes you learn ownership
  before anything works. AQL's one cliff candidate is forward
...
- **Two macro systems.** `macro_rules!` vs proc macros is two
  sub-languages. AQL macros are ordinary AQL at expansion time with
...
- **Trait coherence (orphan rules).** Global open dispatch forces
  ...
  ADR-001's no-shadowing rule as the coherence story, or AQL inherits
  the orphan-rule problem.
- **Async coloring.** Rust split its ecosystem into sync/async
  functions. AQL's `await`-over-lists model is colorless — keep the
...
- **`comptime` rests on types being first-class values manipulated by
  ordinary code** — verbatim AQL doctrine. Zig generics are functions
  returning types; AQL's `Box of [Integer]` is ordinary execution
...
- **Error sets — the strongest language-level idea in this report
  ...
  inferred). AQL errors carry code atoms but nothing enumerates them:
...
- **Checked arithmetic as the only sane default.** Zig makes overflow
  loud and defined. AQL's overflow Phase 0 (uniform checked erroring
...
- **Strictness that fights the user.** Zig's hard errors on unused
  locals are widely resented. AQL's info-severity, never-gating
  advisory convention is the right calibration: gate on wrongness,
  advise on smell.
- **No privacy.** Feldman missed Rust's private fields in Zig. AQL
...
- **Platform/application split ↔ capabilities + policy.** Roc's
  ...
  host platform — is AQL's capability system approached from the
  opposite pole: Roc makes the host mandatory; AQL makes the host's
...
- **Formalize the platform contract (R2).** Roc platforms declare, in
  ...
  against it. AQL has the pieces — capability flags, policy
  profiles, `check` — but no static composition:
  `aql check --platform <profile>` verifying a program touches only
...
- **Functional-but-in-place.** Roc's Perceus-style opportunistic
  ...
  AQL's immutable Nodes pay real copy costs (visible throughout the
...
- **The novelty budget.** Lambda sets — Roc's cleverest inference —
  ...
  before they ossify. AQL's novel core (forward collection,
...
- **Purity absolutism.** Roc programs can't do anything without a
  platform, which taxes casual scripting. AQL's default-open CLI with
...
- **Agent-first self-documentation.** Faber's `llms.txt` + corpus +
  `faber explain` is the same bet AQL made with `aql describe` /
  `aql help` generated from the live engine, the AGENTS.md router,
  and the vault's agent-delegation design (scoped capability tokens,
  the `mcp` broker). AQL's variant is arguably deeper — describe
...
- **Semantic core vs renderings ↔ ADR-007.** "No secondary parsing;
  every AQL structure is macro-constructable Node data" is the same
  ...
  AQL deliberately keeps one surface.
- **Design laws ↔ ADRs.** "Nine design laws govern every language
  decision" is AQL's ADR discipline under another name.
- **Frame types ↔ capabilities/`FileOps`.** Typed I/O boundaries
  between program and host are AQL's capability + embedder-interface
...
- **`aql explain <code>` (R4).** Faber ships `faber explain SEM001`
  (as Rust ships `rustc --explain E0308`). AQL errors already carry
  stable codes (`[aql/incomparable]`, `[aql/cap_denied]`, …) and
  `aql policy explain` already explains *policy* decisions — but
  nothing explains error codes. A small `explain` surface (or
  `describe aql/incomparable`) rendering cause, example, and fix
  guidance from the same live-engine registry would complete the
  loud-failures story cheaply.
- **A machine-readable docs surface (R5).** `aql describe --json`
  plus a generated `llms.txt`-style index (served by the registry
  and/or checked into `docs/`) would give agents Faber's
  fetch-without-install path while keeping AQL's can't-drift
  property, since both render from the live registry.
- **An agent-skills manifest.** AQL already has AGENTS.md for
...
- **"Reviewable output" as a stated property.** Faber markets
  ...
  a feature. AQL's analogs (bytecode disassembly, emitter goldens)
...
- **Rendering multiplicity multiplies the contract.** Every extra
  ...
  backpassing. AQL's single-surface + canonical-`fmt` position is
...
| Axis | Rust | Zig | Roc | Faber (per site) | AQL |
|---|---|---|---|---|---|
...
| Metaprogramming | two macro systems | comptime, no macros | none (deliberate) | semantic staging | hygienic macros = ordinary AQL at expansion time |
...
| R2 | **Checkable platform/embedding contract**: `aql check --platform <profile>` verifying a program against a host's capability + word surface | Roc (platforms) + Faber (frame types) | check + policy machinery | medium |
...
| Pitfall | Source | AQL exposure point |
|---|---|---|
...
| Unexercised paths hide bugs (lazy analysis / off-corpus blindness) | Zig + AQL's own miscompile hunt | whole-program check, ADR-008, widen the differential corpus |
...
AQL's philosophical nearest neighbour is **Zig** (one language at both
...
first fellow traveller aimed at the same *agent-ready* niche AQL
occupies — its ideas are mostly ones AQL already holds in stronger
