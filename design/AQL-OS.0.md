...
Go's `os` package spans files, processes, environment and host interaction. Node's `os` module is narrower and primarily exposes host facts. AQL should follow the narrower model.

`aql:os` is not a dumping ground for every host API. It exposes portable operating-system facts, conventional directories, process metadata, feature probing and safe process-lifecycle integration.

File operations remain in AQL's I/O facilities. Path manipulation remains in the path module. Networking remains in networking modules. Process execution belongs in a separate, capability-gated execution facility.
...
- Signal delivery never executes arbitrary AQL code inside a native signal handler.
...
Basic current-user metadata may be exposed when the host can provide it, but authentication and biometric identity do not belong here. Host keychains, user verification, passkeys and hardware-bound keys are deferred to a future `aql:trust` design.
...
The default API maps process termination events to structured AQL lifecycle behaviour:

- interrupt and terminate cancel the root or selected execution scope;
- cancellation includes a structured signal reason;
- service code performs orderly shutdown through ordinary AQL control flow;
...
- AQL code observes the event later on the normal scheduler;
- subscriptions have explicit lifetime and cleanup;
- duplicate/coalesced delivery semantics are documented.

AQL callbacks are never invoked directly from a POSIX signal handler.
...
`aql:os` provides lifecycle events; structured task and execution-context facilities provide the shutdown mechanics. Traditional Unix daemonization such as fork/session/double-fork is not a default portable abstraction and may be left to a host-specific service layer.
...
6. Whether raw signal controls live under `aql:os` with capabilities or a separate host module.
