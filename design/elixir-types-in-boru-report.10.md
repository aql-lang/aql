...
| Gradual static+dynamic | Carrier-based abstract interpreter (`boru check`), mapped explicitly to abstract interpretation | `design/CARRIER-STATIC-TYPECHECK-REPORT.10.md`, `eng/go/carrier.go` |
...
(`eng/go/value.go`, for diagnostics/traces) and in the `boru check`
residual stack (`lang/go/boru.go`) — so the gradual modality is legible
...
`boru check` with a new `index_out_of_range` diagnostic.
...
zero-runtime-cost `boru check` diagnostic that reuses existing
