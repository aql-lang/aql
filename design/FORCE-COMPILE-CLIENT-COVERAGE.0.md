...
| `fn [[a:Integer][Map][ {x:[a]} ]]` | ❌ `fn f: body result of unknown provenance` (emit.go:1442) | A computed map whose value comes from a **fn frame-local**, returned as the **body result**, loses provenance. |
| `fn [[a:Integer][Map][ do {x:[a]} ]]` | ❌ `unannotated or opaque word do` (emit.go:2004) | `do {computed-map}` types its **output** as a dynamic carrier, so the dynamic-output gate refuses it. |
